...
export const SankeyChart:React.FC<ISankeyChartProps>=({
    ...
    const MyCustomNode=useCallback(({payload,...rest}:any):React.ReactElement<SVGElement>=>{
         ...
         */
        const renderLegend=useCallback((name:string,value:string)=>(
         ...
         */
        const legendXPosition=useMemo(():number=>rest.x+payload.dx,[rest.x,payload.dx])
         ...
         */
        const legendYPosition=useMemo(():number=>rest.y+payload.dy/2-(heightOffset?heightOffset/2:INITIAL_LEGEND_HEIGHT_OFFSET),[rest.y,payload.dy,heightOffset])
